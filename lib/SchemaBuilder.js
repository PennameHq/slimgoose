...
const { inSerial } = require('../utils/promises')
...
class SchemaBuilder {
	...
	/**
	 * Adds key path / schema type pairs to the schema.
	 * See https://mongoosejs.com/docs/api.html#schema_Schema-add
	 * @param {Object|mongoose.Schema} fieldsMap
	 */
	add(fieldsMap) {
		this._schema.add(fieldsMap)
		return this
	}
	...
	_decorateMethods(methodsMap, type) {
		...
		return Object.entries(methodsMap).reduce((map, [key, staticMethod]) => {
			map[key] = function () {
				return builder
					._runPreMethodByKey(key, builder._getPreMethodByKey(key, type), arguments, this)
					.then((newArgs) => {
	 ...
	 */
	_runPreMethodByKey(key, funcs, args, context) {
		let newArgs = undefined
		return inSerial(funcs, {
			data: {
				key,
				args,
				context,
				setNewArguments(_newArgs) {
					newArgs = _newArgs
				},
			},
		}).then(() => {
